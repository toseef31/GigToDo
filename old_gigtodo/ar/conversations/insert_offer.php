...
$insert_offer = $db->insert("messages_offers",array("sender_id" => $login_seller_id,"proposal_id" => $proposal_id,"request_id"=>$request_id,"description" => $description,"delivery_time" => $delivery_time,"amount" => $amount,"status" => 'active'));
...
if($insert_offer){
	...
	if($update_inbox_sellers){
		...
		if($login_seller_id != $receiver_seller_id){
	  	...
	  	try{
			...
			<center>
			<a href='$site_url/ar/conversations/inbox?single_message_id=$message_group_id' class='btn'>View & Reply</a>
			</center>
			</div>
			</div>
			</body>
