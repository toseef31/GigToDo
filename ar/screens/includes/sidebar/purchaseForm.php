<form action="../../checkout" id="checkoutForm" method="post">
  ...
  <div class="header">
    ...
    <div class="price <?=($lang_dir == "right" ? 'text-right':'')?>">
      <b class="currency"><?= $s_currency; ?><span class="total-price"><?= $proposal_price; ?></span></b>
    </div>
  </div>
  <hr class="mt-0">
  <div class="row">
    <div class="col-12 ml-2 p-2">
      <h6 class="mb-0 <?=($lang_dir == "right" ? 'text-right pr-4':'')?>"><i class="fa fa-clock-o"></i> <?= $delivery_proposal_title; ?> Delivery </h6>
    </div>
  </div>
  <?php 
    include('extras.php');
