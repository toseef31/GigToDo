...
if(!filter_var($response["success"],FILTER_VALIDATE_BOOLEAN)){
    ...
    header("Location:"."/ar/weaccept_order?checkout_seller_id=$payment->seller_id&proposal_id={$_SESSION['c_proposal_id']}&proposal_qty={$_SESSION['c_proposal_qty']}&proposal_price={$_SESSION['c_sub_total']}&$extras&$minutes");

}else{
    header("Location:/ar/payments?payment_failed=true&payment_type=wallet");
}
