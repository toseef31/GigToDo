...
while($row_notifications = $select_notofications->fetch()){
	...
	$data['notifications'][$i]['sender_image'] = "user_images/" . @$row_sender->seller_image;
	if(empty($row_sender->seller_image)){ $data['notifications'][$i]['sender_image'] = "user_images/empty-image.png"; }
	...
	if(strpos($sender_id,'admin') !== false){
		...
		$data['notifications'][$i]['sender_image'] = "admin/admin_images/$admin_image";
	}
