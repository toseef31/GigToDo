...
$s_value = $keyword;
$get_proposals = $db->query("select * from proposals where proposal_title like '%$keyword%' AND proposal_status='active'");
// print_r($get_proposals);
// $get_proposals = $db->select("proposals",array("proposal_title"=>$keyword,"proposal_status"=>'active'));;
// print_r($get_proposals);
while($row_proposal = $get_proposals->fetch()){
	// print_r($row_proposal);die();
