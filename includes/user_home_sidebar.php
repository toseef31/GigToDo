<div class="card rounded-0 mb-3 welcome-box"><!-- card rounded-0 mb-3 welcome-box Starts -->
	...
	<img src="user_images/<?= $seller_image; ?>" class="img-fluid rounded-circle mb-3">
	<?php }else{ ?>
	...
	<div class="row m-0"><!--- row Starts --->
		<div class="col-lg-6 m-0 p-0 pr-2 pb-lg-0 pr-lg-2 pb-md-2 pr-sm-2"><!--- col-md-6 Starts --->
			<h5><a href="<?= $site_url; ?>/dashboard"><?= $lang['dashboard']['title']; ?></a></h5>
			<h5><a href="<?= $site_url; ?>/proposals/create_proposal"><?= $lang['user_home']['add_proposal']; ?></a></h5>
			<h5 class="mb-0"><a href="<?= $site_url; ?>/requests/post_request"><?= $lang['post_request']; ?></a></h5>
		</div><!--- col-md-6 Ends --->
		<div class="col-lg-6 m-0 p-0 pl-2 pt-lg-0 pl-lg-2 pl-md-0 pt-md-2 pl-sm-2"><!--- col-md-6 Starts --->
			...
			<h5 class="mb-0"><a href="<?= $site_url; ?>/settings"><?= $lang['settings']; ?></a></h5>
		</div><!--- col-md-6 Ends --->
	</div><!--- row Ends --->
	<hr>
	<h5><a href="<?= $site_url; ?>/customer_support">Contact <?= $site_name; ?></a></h5>
	</div><!-- card-body Ends -->
</div><!-- card rounded-0 mb-3 welcome-box Ends -->
<div class="rounded-0 carosel_sec">
	<h3 class="buy_head <?=($lang_dir == "right" ? 'text-right':'')?>"><?= $lang['buy_it_again']; ?></h3>
		<?php 
		$select_orders = $db->query("select DISTINCT proposal_id from orders where buyer_id='$login_seller_id' AND order_status='completed'");
		$count_orders = $select_orders->rowCount();
		if($count_orders == 0){
		echo "<p class='text-muted'><i class='fa fa-frown-o'></i> {$lang['no_buy_it_again']} </p>";
		}else{ 
		?>
  <div id="demo" class="carousel slide" data-ride="carousel"><!-- The slideshow -->
	   ...
	   <div class="proposal-card-base mp-proposal-card"><!--- proposal-card-base mp-proposal-card Starts --->
	     ...
	     <img src="user_images/<?= $seller_image; ?>" class="rounded-circle" width="32" height="32">
	     </span>
	     ...
	     <footer class="proposal-card-footer"><!--- proposal-card-footer Starts --->
	      ...
	      <div class="proposal-price">
		      <a class="js-proposal-card-imp-data">
		      	<small>Starting At</small><?= $s_currency; ?><?= $proposal_price; ?>
		      </a>
	      </div>
...
<h3 class="buy_head <?=($lang_dir == "right" ? 'text-right':'')?>"><?= $lang['recently_viewed']; ?></h3>
  <?php
...
<div class="proposal-card-base mp-proposal-card"><!--- proposal-card-base mp-proposal-card Starts --->
	...
	<?php } ?>
	<?php
  $select_recent = $db->query("select * from recent_proposals where seller_id='$login_seller_id' order by 1 DESC LIMIT  1,4");
  while($row_recent = $select_recent->fetch()){
  $proposal_id = $row_recent->proposal_id;
  $get_proposals = $db->query("select * from proposals where proposal_id='$proposal_id' AND proposal_status='active'");
  $count_proposals = $get_proposals->rowCount();
...
<div class="card rounded-0 sticky-start mb-3 card_user">
	...
	<h4><?= $lang['start_selling']['title']; ?></h4>
	<p><?= $lang['start_selling']['desc']; ?></p>
	<button onclick="location.href='start_selling'" class="btn get_btn"><?= $lang['start_selling']['button']; ?></button>
	</div>
</div>
<br>
<script>
